...
class Game:
    ...
    def sort_players_based_on_experience(self):
        self.__players_list=sorted(self.__players_list,key= lambda x:x.get_experience(),reverse=True)
        
    def shift_player_to_new_position(self,old_index_position,new_index_position):
        x=abs(new_index_position-old_index_position)
        self.__players_list=self.__players_list[-x:]+self.__players_list[:-x]
